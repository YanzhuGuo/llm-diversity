...
for model_id in ["google/gemma-2-9b-it", "Qwen/Qwen2.5-7B-Instruct", "meta-llama/Meta-Llama-3.1-8B-Instruct", "allenai/OLMo-7B-Instruct-hf", "facebook/opt-6.7b", "tiiuae/falcon-7b-instruct", "mistralai/Mistral-Nemo-Instruct-2407"]:
    ...
    model = AutoModelForCausalLM.from_pretrained(model_id, torch_dtype=torch.bfloat16).to("cuda:1")
    ...
    if "opt" not in model_id and "falcon" not in model_id:
        messages = [[{"role": "user", "content": "You are a professional story writer, you always show the story without any additional explanations or format changes. Continue the following story.\n\n"+p}] for p in prompts]
        prompts = tokenizer.apply_chat_template(messages, tokenize=False, add_generation_prompt=True)
        
    else:
        prompts = ["You are a professional story writer, you always show the story without any additional explanations or format changes. Continue the following story:\n\n"+p for p in prompts]
    ...
    for i in range(0, len(prompts), batch_size):
        ...
        output_sequences = model.generate(**inputs, max_new_tokens=512, do_sample=True, temperature=0.6, top_p=0.9, repetition_penalty=1.02)
